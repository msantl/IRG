...
vector< point_t > izracunajProjekciju(const vector< point_t > &v);
vector< point_t > izracunajBeziera(void);
...
void renderScene() {
  ...
  // nacrtaj beziera
  vector< point_t > bezier = izracunajBeziera();

  vector< point_t > p1 = izracunajProjekciju(putanja);
  vector< point_t > p2 = izracunajProjekciju(bezier);

  // crveno zadani segmenti
  ...
  // zeleno interpolacijska krivulja
  ...
  vector< point_t > projekcija = izracunajProjekciju(vrh);
...
vector< point_t > izracunajBeziera(void) {
  vector< point_t > bezier;

  int N = putanja.size() - 1;

  for (double t = 0; t <= 1; t += 0.01) {
    point_t T = point_t(0, 0, 0, 0);
    int i = 0;
    for (
      vector< point_t >::iterator
      it = putanja.begin();
